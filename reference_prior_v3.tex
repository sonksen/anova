...
\eqref{eq:ref_l}. We define $\pmb\theta_{(1:j),\;k}$ to be the $k^{th}$
element of the vector $\pmb\theta_{(1:j)}$. The term
$|h_j(\boldsymbol{\theta})|^{1/2}$ can be canceled out from Equation
\eqref{eq:ref_l} because it is only a function of $n$. Under regularity
...
Then the integral within the product is
\begin{eqnarray*}
  \int_{\Theta^{l}\cap[\Theta_{j}|\Theta_{(1:(j-1))}]}
|h_j(\boldsymbol{\theta})|^{1/2}   
